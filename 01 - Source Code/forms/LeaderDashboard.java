...
public class LeaderDashboard {
	...
	List<User> members;
	Iterator<User> it1;
	JButton btn_viewFullProject, btn_addMembers, btn_assignTask, btn_profile;
	// profile
	...
	LeaderDashboard(User user) {
		...
		btn_createNewProject = new JButton("New Project");
		btn_createNewProject.setFont(frame.myFont);
		btn_createNewProject.setBounds(1350, 80, 150, 30);
		...
		lbl_passwordValue.setBounds(950, 560, 250, 30);
		...
		btn_viewFullProject.setBounds(730, 900, 230, 30);
		btn_viewFullProject.setVisible(false);

		btn_addMembers = new JButton("Add New Members");
		btn_addMembers.setFont(frame.myFont);
		btn_addMembers.setBounds(980, 900, 250, 30);
		btn_addMembers.setVisible(false);

		btn_assignTask = new JButton("Assign Tasks");
		btn_assignTask.setFont(frame.myFont);
		btn_assignTask.setBounds(1250, 900, 200, 30);
		btn_assignTask.setVisible(false);

		change(true, false);
		// adding components
		frame.add(lbl_welcome);
		frame.add(btn_createNewProject);
		frame.add(lbl_title);
		...
		frame.add(btn_addMembers);
		frame.add(btn_assignTask);
		...
		lst_projectList.addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				if (!e.getValueIsAdjusting()) {
					...
					lbl_projectLeaderValue.setText(Integer.toString(user.getUserId()) + " - " + user.getName());
					...
					if (members.size() != 0) {
						...
						lbl_memberTemp = new JLabel("---");
						lbl_memberTemp.setFont(frame.myFont);
						pnl_projectMembersValue.add(lbl_memberTemp);
					}
					btn_viewFullProject.setVisible(true);
					btn_addMembers.setVisible(true);
					btn_assignTask.setVisible(true);
		...
		btn_viewFullProject.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				new LeaderProjectInfo(user, projectList.get(lst_projectList.getSelectedIndex()));
				frame.dispose();

			}
		});
		// assign task
		btn_assignTask.addActionListener(new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent e) {
				new TaskManagement(user, projectList.get(lst_projectList.getSelectedIndex()));
				frame.dispose();
			}
		});
		// add members
		btn_addMembers.addActionListener(new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent e) {
				new MemberManagement(user, projectList.get(lst_projectList.getSelectedIndex()));
				frame.dispose();
			}
		});

		// new project
		btn_createNewProject.addMouseListener(new MouseAdapter() {
			public void mousePressed(MouseEvent e) {
				new NewProject(user);
				frame.dispose();
	...
	public void change(boolean one, boolean two) {
		...
		btn_addMembers.setVisible(two);
		btn_assignTask.setVisible(two);
